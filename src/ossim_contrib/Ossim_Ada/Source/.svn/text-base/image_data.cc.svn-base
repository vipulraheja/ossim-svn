...
void demo1(const ossimFilename& filename)
{
   ...
   //
   ossimImageData* data = handler->getTile(regionsOfInterest);
   ...
   if(data)
   {
      ...
      cout << "Pixel scalar type = "
           << ossimScalarTypeLut().getEntryString(data->getScalarType())
           << endl;
      ...
      if(data->getDataObjectStatus() == OSSIM_FULL)
      {
         ...
         for(int bandIndex = 0; bandIndex < data->getNumberOfBands(); ++bandIndex)
         {
...
void demo2(const ossimFilename& filename)
{
   ...
   //
   ossimImageData* data = ossimImageDataFactory::instance()->create(NULL,
								    handler);
   ...
   //
   ossimImageData* dupData = (ossimImageData*)data->dup();
   ...
   //
   ossimImageData* data2 = new ossimImageData(NULL, // owner of the data object
...
void demo3(const ossimFilename& filename)
{
   ...
   //
   ossimImageData* data = handler->getTile(ossimIrect(10,
						      10,
						      10 + 127,
						      10 + 127));
   ...
   if(data)
     {
       ...
       ossimImageData* data2 = ossimImageDataFactory::instance()->create(0,
									 handler);
       ...
       //
       for(ossim_int32 band = 0; band < data->getNumberOfBands(); ++band)
	 {
