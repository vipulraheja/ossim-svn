...
//
//----------------------------------------------------------------------------
// $Id: ossimGeoPdfReader.cpp 1261 2010-08-10 15:37:52Z ming.su $
...
ossimString getValueFromMap(std::map<ossimString, ossimString> info, ossimString key)
{
  ossimString valueStr;
  std::map<ossimString, ossimString>::iterator it = info.find(key);
  if (it != info.end())
  {
    valueStr = it->second;
  }
...
void ossimGeoPdfReader::resetCacheBuffer(ossimFrameEntryData entry)
{
  ...
  ossim_int32 width = frameWidthVector[col];
  ossim_int32 height = frameHeightVector[row];
...
bool ossimGeoPdfReader::open()
{
   ...
   if (m_pdfMemDocument != NULL)
   {
     ...
     for(int i = 0; i < count; i++)
     {
       ...
       if( pdfPage ) 
       {
         ...
         m_pageVector[i] = new ossimGeoPdfVectorPageNode();
         m_pageVector[i]->setGeoImage(geoImage);
         m_pageVector[i]->setBoundingBox(computeBoundingRect(geoImage));
   ...
   theTile->initialize();
   m_cacheTile = (ossimImageData*)theTile->dup();
   m_cacheTile->initialize();

   m_cacheSize.x = frameWidthVector[0];
   m_cacheSize.y = frameHeightVector[0];
...
bool ossimGeoPdfReader::setCurrentEntry(ossim_uint32 entryIdx)
{
  ...
  {
    if ((ossim_int32)m_pageVector.size() > entryIdx)
    {
      theGeometry = 0;
      m_imageRect.makeNan();
...
void ossimGeoPdfReader::closeEntry()
{
   ...
   m_podofoTileInfo.clear();
   frameWidthVector.clear();
   frameHeightVector.clear();
...
void ossimGeoPdfReader::buildFrameEntryArray()
{
  ...
  if (m_podofoImageObjs.size() > 0 && m_podofoTileInfo.size() > 0)
  {
    ...
    for (col = 0; col < m_numOfFramesHorizontal; ++col)
    {
      ...
      PoDoFo::PdfImage image(tmpObj);
      int width = image.GetWidth();
      frameWidthVector[col] = width;
    }
   
    std::map<ossim_int32, ossim_int32>::iterator  itWidth = frameWidthVector.begin();
    while (itWidth != frameWidthVector.end())
    {
      m_numberOfSamples = m_numberOfSamples + itWidth->second;
      itWidth++;
    }

    std::map<ossim_int32, ossim_int32>::iterator  itHeight = frameHeightVector.begin();
    while (itHeight != frameHeightVector.end())
    {
    ...
    {
      m_numberOfBands = m_frameEntryArray[0][0].size();
    }
    else
    {
      ...
      m_numberOfBands = length/(frameHeightVector[0]*frameWidthVector[0]);
    }
  }
}
...
std::vector<ossimGeoPdfReader::ossimFrameEntryData> ossimGeoPdfReader::getIntersectingEntries(const ossimIrect& rect)
{
  ...
  if(rect.intersects(imageRect))
  {
    ...
    ossimIrect frameRect(clipRect.ul().x/frameWidthVector[0],
      clipRect.ul().y/frameHeightVector[0],
      clipRect.lr().x/frameWidthVector[0],
      clipRect.lr().y/frameHeightVector[0]);
    ...
    for(ossim_int32 row = frameRect.ul().y; row <= frameRect.lr().y; ++row)
    {
      for(ossim_int32 col = frameRect.ul().x; col <= frameRect.lr().x; ++col)
      {
        ...
        {
          pixelRow = pixelRow + frameHeightVector[iRow];
        }
        for (ossim_int32 iCol = 0; iCol < col; ++iCol)
        {
          pixelCol = pixelCol + frameWidthVector[iCol];
        }
...
ossimProjection* ossimGeoPdfReader::getProjectionFromStr(ossimString projContents)
{
  ...
  std::map<ossimString, ossimString> projectionMap;
