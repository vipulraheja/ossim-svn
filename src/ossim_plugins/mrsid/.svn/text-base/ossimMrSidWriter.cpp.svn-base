...
void ossimMrSidWriter::writeMetaDatabase(LTIMetadataDatabase& metadataDatabase)
{
  ...
  if (!mapProj)
  {
    ossimNotify(ossimNotifyLevel_WARN)
      << "ossimMrSidWriter::writeMetaDatabase WARNING:"
      << "\nProjection failed to cast to map proection!"
      << std::endl;
    return;  
  }
  ...
  {
    ossim_uint16 theGcsCode = mapProj->getGcsCode();
    //if (theGcsCode == 0)
    //{
    //  theGcsCode = ossimGcsCodeProjectionFactory::instance()->getGcsCodeFromProjection(mapProj);
    //}
    int code = theGcsCode;

    LTIMetadataRecord mrGeoCSType( "GEOTIFF_NUM::2048::GeographicTypeGeoKey",
      LTI_METADATA_DATATYPE_SINT32, &code);
  ...
  {
    ossim_uint16 thePcsCode = mapProj->getPcsCode();
    //if (thePcsCode == 0)
    //{
    //  thePcsCode = ossimEpsgProjectionDatabase::instance()->findProjectionCode(mapProj);
    //}
    int code = thePcsCode;
