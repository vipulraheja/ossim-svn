...
//
//----------------------------------------------------------------------------
// $Id: ossimPngWriter.cpp 19730 2011-06-06 21:46:30Z dburken $
...
//---
// For the "ident" program which will find all exanded $Id: ossimPngWriter.cpp 19730 2011-06-06 21:46:30Z dburken $ macros and print
// them.
//---
#if OSSIM_ID_ENABLED
static const char OSSIM_ID[] = "$Id: ossimPngWriter.cpp 19730 2011-06-06 21:46:30Z dburken $";
#endif
...
bool ossimPngWriter::writeStream()
{
   ...
   if(theInputConnection->isMaster())
   {
      ...
      if ( bands == 1 )
      {
         theTransparentGray = theInputConnection->getNullPixelValue();
      }
      else
      if ( bands == 3 )
      {
         theTransparentRed  = theInputConnection->getNullPixelValue(0);
         theTransparentGray = theInputConnection->getNullPixelValue(1);
         theTransparentBlue = theInputConnection->getNullPixelValue(2);
      }
                               ...
                               const char* prefix)
{
   ...
   if(value)
   {
      ossimString addAlphaChanStr( value );
      bool bAddAlphaChan = addAlphaChanStr.toBool();
      setAlphaChannelFlag( bAddAlphaChan );
   }
...
void ossimPngWriter::setProperty(ossimRefPtr<ossimProperty> property)
{
   ...
   {
      ossimString addAlphaChanStr = property->valueToString();
      bool bAddAlphaChan = addAlphaChanStr.toBool();
      setAlphaChannelFlag( bAddAlphaChan );
   }
   else
