...
//
//*******************************************************************
//  $Id: ossimGdalTileSource.cpp 19733 2011-06-06 23:35:25Z dburken $
...
bool ossimGdalTileSource::open()
{
   ...
   if (theSubDatasets.size() == 0)
   {
      ...
      if (driverTmp != 0)
      {
         ossimString driverNameTmp = ossimString(GDALGetDriverShortName(driverTmp));
...
ossimRefPtr<ossimImageGeometry> ossimGdalTileSource::getInternalImageGeometry() const
{
   ...
   {
      //---
      // AIG driver considered pixel is area or upper left
      // corner of tie point.  So shift tie to center of pixel.
      ///---
...
{
   //---
   // Currently same as base ossimImageHandler::getNumberOfDecimationLevels but left in place
   // in case at some point we start utilizing gdal overviews.
   //---
   ossim_uint32 result = 1; // Start with one.
