                                                 ...
                                                 const char* prefix) const
{
   ...
   if ( !_sceneCoord )
   {
      if(traceDebug())
      {
         ossimNotify(ossimNotifyLevel_WARN)
            << MODULE << " ERROR: _sceneCoord is NULL!\n"
        ...
        ( _SrToGr_exponent.size() == _SrToGr_coeffs.size()) )
   {
      ...
      const std::vector<int>::size_type COUNT = _SrToGr_exponent.size();

      kwl.add(prefix, NUMBER_SRGR_COEFFICIENTS_KW, static_cast<ossim_uint32>(COUNT));
      
      for(std::vector<int>::size_type i = 0; i < COUNT; ++i)
      {
         ossimString iStr = ossimString::toString( static_cast<ossim_uint32>(i) );
         ossimString kw = kw1;
         kw += iStr;
