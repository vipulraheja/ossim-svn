...
ossimGpt ossimFormosatDimapSupportData::createGround(const ossimString& s)const
{
   ...
   s.split(v, ossimString(" "));

   ossimGpt gpt;
   
   if ( v.size() >= 4 )
   {
      gpt.lat = v[0].toFloat64();
      gpt.lon = v[1].toFloat64();
      gpt.hgt = v[2].toFloat64();
      const ossimDatum* d = ossimDatumFactory::instance()->create(v[3]);
      if ( d )
      {
         gpt.datum(d);
      }
