...
//                                                           deltaLat));
         ...
         theProjection->setUlTiePoints(ossimGpt(tileExtents->getMaxLat()-(gsd.y*.5), 
                                                tileExtents->getMinLon()+(gsd.x*.5)));
         theProjection->update();
         theViewInterface->setView(theProjection.get());
...
ossimPlanetTextureLayerStateCode ossimPlanetOssimImageLayer::buildChain()
{
   ...
   if(theSource.valid())
   {
      ...
      theImageSpaceChain.push_back(theSource.get());
...
//      theProjection = new ossimEquDistCylProjection;
      if(!theViewInterface)
      {
         ...
         theRenderer->setView(theProjection.get());
         theRenderer->getResampler()->setFilterType(theFilterType);
