...
thePriorityPoint(0.0,0.0,0.0)
{
   ...
   theMaxNumberOfOperationsToApplyToGraphPerFrame = 50;
...
//   osg::Timer_t tick = osg::Timer::instance()->tick();
   ...
   std::cout << "CULL COUNT ===== " << count.theCullCount << std::endl;
   std::cout << "NODE COUNT ===== " << count.theNodeCount << std::endl;
#endif
...
{
   OpenThreads::ScopedLock<OpenThreads::Mutex> lock(theNeedToCompileQueueMutex);
   if(thePrecompileEnabledFlag)
   {
      theNeedToCompileQueue.push_back(request);
      setRedrawFlag(true);
   }
   else
   {
