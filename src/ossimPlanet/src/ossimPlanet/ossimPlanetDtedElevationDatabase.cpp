         ...
         if(dtedFile.valid())
         {
            ...
            for(idxPts = 0; idxPts < nPoints; ++idxPts,++optimizedOutPtr)
            {
               if((*bufPtr == OSSIMPLANET_NULL_HEIGHT)&&
                  (optimizedOutPtr->y() >= dtedFile->theMinLat)&&
                  (optimizedOutPtr->y() <= dtedFile->theMaxLat)&&
                  (optimizedOutPtr->x() >= dtedFile->theMinLon)&&
                  (optimizedOutPtr->x() <= dtedFile->theMaxLon))
               {
                  double h = dtedFile->theHandler->getHeightAboveMSL(ossimGpt(optimizedOutPtr->y(),
                                                                              optimizedOutPtr->x()));
                  if(!ossim::isnan(h)&&
                     (h!=nullHeight))
                  {
