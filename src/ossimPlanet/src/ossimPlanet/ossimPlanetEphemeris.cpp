...
//            (*color)[c].set(bottom_color[i][0], bottom_color[i][1], bottom_color[i][2], 1.0-(bottom_color[i][0]+bottom_color[i][1]+bottom_color[i][2])/3.0);
//            (*color)[c+19].set(lower_color[i][0], lower_color[i][1], lower_color[i][2], 1.0-(lower_color[i][0]+lower_color[i][1]+lower_color[i][2])/3.0);
//            (*color)[c+19+19].set(middle_color[i][0], middle_color[i][1], middle_color[i][2], 1.0-(middle_color[i][0]+middle_color[i][1]+middle_color[i][2])/3.0);
//            (*color)[c+19+19+19].set(upper_color[i][0], upper_color[i][1], upper_color[i][2], 1.0-(upper_color[i][0]+upper_color[i][1]+upper_color[i][2])/3.0);
//            (*color)[c+19+19+19+19].set(center_color[0], center_color[1], center_color[2], 1.0-(center_color[0]+center_color[1]+center_color[2])/3.0);
            c++;
         }
...
//      std::cout << "density before = " << density << "\n"
//      << "density after   = " << fade_to_black(density, theEyeLlh[2], theMaximumAltitudeToShowFog) << std::endl;
...
//      osg::Vec4 d2(1, 1, 1, 1.0);
 ...
 //     osg::Vec4 a2(1, 1, 1, 1.0);
   ...
   void updateFogColor()
   {
      ...
      float sif = 0.5f - cosf(osg::DegreesToRadians(theSunElevation) * 2.f) / 2.f + 0.000001f;
   ...
   //   osg::StateSet* state = theObjectGroup->getOrCreateStateSet();
   }
...
//   osg::ref_ptr<InterpTable> mAmbLightTable;
//   osg::ref_ptr<InterpTable> mDifLightTable;
//   osg::ref_ptr<InterpTable> mSpecLightTable;
...
//   osg::Vec3 mSpecLightColor; ///<The current specular light color
  ...
  // osg::ref_ptr<osg::MatrixTransform> theScaleTransform;
   osg::ref_ptr<osg::MatrixTransform> theSkyDomeTransform;
   osg::ref_ptr<SkyDome>            theSkyDome;
//   osg::ref_ptr<SkyDomeShader>        theShaders;
   
   //osg::ref_ptr<MoveEarthySkyWithEyePointTransformAzimuth> theMoveTransform;
  // osg::ref_ptr<MoveEarthySkyWithEyePointTransform> theMoveTransform;
   /**
...
//   theObjectGroup->addChild(theLightGroup.get());
   osg::Geode* geode = new osg::Geode();
  ...
  //theMoonPointModel->setNode(geode);
//   theMoonPointModel->setCullingActive(false);
//   theMoonPointModel->lsrSpace()->setCullingActive(false);
//   theMoonPointModel->getOrCreateStateSet()->setMode(GL_LIGHTING, osg::StateAttribute::ON);
  
   theSunBillboard = createPlanetBillBoard(1, 
                                           "Sun",
                                           "");
   theSunBillboard->setMinPixelSize(64);
   theSunBillboard->setMaxPixelSize(64);
   theSunPointModel->setName("SunPointModel");
   theSunPointModel->setNode(theSunBillboard.get());
   theSunPointModel->setLayer(theLayer);
   theSunBillboard->setCullingActive(false);
   theSunPointModel->setCullingActive(false);
   theSunPointModel->lsrSpace()->setCullingActive(false);
   theSunPointModel->getOrCreateStateSet()->setMode(GL_BLEND, osg::StateAttribute::ON);
   theSunPointModel->getOrCreateStateSet()->setMode(GL_LIGHTING, osg::StateAttribute::OFF);
  ...
  // mAdvFogCtrl.set(1.f, 10.f, 2.545f); // T, E, N
   ...
   theObjectGroup->addChild(theSunPointModel.get());
   theObjectGroup->addChild(theMoonGroup.get());
//   theObjectGroup->addChild(theMoonPointModel.get());
//   theObjectGroup->addChild(theMoonModel.get());
...
void ossimPlanetEphemeris::EphemerisData::setMembers(ossim_uint64 membersBitMap)
{
   ...
   {
      theSunPointModel->setNodeMask(0xffffffff);
      theSunPointModel->setEnableFlag(true);
   }
   else
   {
      theSunPointModel->setNodeMask(0x0);
      theSunPointModel->setEnableFlag(false);
   }
...
void ossimPlanetEphemeris::EphemerisData::updatePositions(osg::NodeVisitor& nv)
{
   ...
   if(theSunPointModel->lsrSpace())
   {
      osg::Vec3d adjSunXyz = theSunXyz - theEyeXyz;
      adjSunXyz.normalize();
      osg::Vec3d newSunPos = theEyeXyz + adjSunXyz*10;
      osg::Vec3d newSunLlh;
      theLayer->theModel->xyzToLatLonHeight(newSunPos, newSunLlh);
     // let's set a false position
      //
      //theSunPointModel->lsrSpace()->setLatLonAltitude(theSunLlh);
      theSunPointModel->lsrSpace()->setLatLonAltitude(newSunLlh);
      //theSunPointModel->lsrSpace()->setXYZ(newSunPos);
      //theSunPointModel->lsrSpace()->setXYZ(newSunPos);
   }
//   if(theMoonPointModel->lsrSpace())
   {
//      theMoonPointModel->lsrSpace()->setLatLonAltitude(theMoonLlh);
      ...
      //
      // theMoonPointModel->lsrSpace()->setLatLonAltitude(theMoonLlh);
//      theMoonPointModel->lsrSpace()->setLatLonAltitude(newMoonLlh);
      //theMoonPointModel->lsrSpace()->setXYZ(newMoonPos);
#endif
   }
...
void ossimPlanetEphemeris::setSunTextureFromImage(osg::Image* texture)
{
   if ( texture )
   {
      theEphemerisData->theSunBillboard->setIcon(theEphemerisData->createBillboardTexture(texture));
   }
}
...
void ossimPlanetEphemeris::setMoonCullCallback(osg::ref_ptr<osg::NodeCallback> cb)
{
   if(theEphemerisData&&theEphemerisData->theMoonGroup.valid())
   {
      theEphemerisData->theMoonGroup->setCullCallback(cb.get());
   }
}
