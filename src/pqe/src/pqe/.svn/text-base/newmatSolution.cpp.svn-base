...
void R1_R1::Set(Real X)
{
   if ((!minXinf && X <= minX) || (!maxXinf && X >= maxX))
       throw(pqe::Exception("X value out of range"));
   x = X; xSet = true;
}

R1_R1::operator Real()
{
   if (!xSet) throw(pqe::Exception("Value of X not set"));
   Real y = operator()();
   return y;
}
...
{
   lim--;
   if (!lim) throw(pqe::Exception("Does not converge"));
   Last = V;
...
Real OneDimSolve::Solve(Real Y, Real X, Real Dev, int Lim)
{
   ...
   for (;;)
   {
      ...
      case start:
         ...
         if (y[U] == y[L])
            throw(pqe::Exception("Function is flat"));
         ...
         for (i=0; i<20; i++)
         {
            ...
            if (y[U] < y[C])
                throw(pqe::Exception("Function is not monotone"));
            Dev *= 2.0;
            State(C,U,L);
         }
         if (TheLoop != start ) break;
         throw(pqe::Exception("Cannot locate a crossing point"));
