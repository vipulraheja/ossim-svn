...
//----------------------------------------------------------------------------
// $Id: Util.cpp 19684 2011-05-31 14:46:56Z dburken $
                                         ...
                                         bool keepAspectFlag)
{
   ...
   ossimRefPtr<ossimImageFileWriter> writer = ossimImageWriterFactoryRegistry::instance()->createWriter(writerType);
...
double oms::Util::imageHeading(const std::string& filename, ossim_int32 entryId)
{
   ...
   if(handler.valid())
   {
      ...
      ossimRefPtr<ossimProjection> proj = createProjection(handler.get());
      if(proj.valid())
      {
         ossimIrect rect = handler->getBoundingRect();
         ossimDpt midPoint = rect.midPoint();
         ossimDpt upPoint = midPoint - ossimDpt(0.0,10);
         ossimGpt wptCenter;
         ossimGpt wptUp;
         proj->lineSampleToWorld(midPoint, wptCenter);
         proj->lineSampleToWorld(upPoint, wptUp);
         ossimDpt p0(wptCenter);
         ossimDpt p1(wptUp);
         ossimDpt delta = p1-p0;
         double len = delta.length();
         if(!ossim::almostEqual(0.0, len))
         {
            delta = delta*(1.0/len);
         }
         delta.lat = ossim::clamp(delta.lat, -1.0, 1.0);
         delta.lon = ossim::clamp(delta.lon, -1.0, 1.0);
         heading = calculateRotationRelativeToY(delta);
      }
   }
