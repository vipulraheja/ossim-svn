...
void ossimQtOverviewBuilderDialog::buildClicked()
{
   ...
   theWriter->setInputSource(theImageHandler.get());
   ...
   theWriter->addListener((ossimProcessListener*)pd); // Progress dialog listens to writer...
   ...
   // Build the overview.
   bool result = theWriter->execute();
   theWriter->removeListener((ossimProcessListener*)pd);
   if (result)
   {
