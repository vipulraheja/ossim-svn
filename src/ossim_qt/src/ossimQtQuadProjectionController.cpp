...
//
//*************************************************************************
// $Id: ossimQtQuadProjectionController.cpp 15766 2009-10-20 12:37:09Z gpotts $

#include <vector>
#include <sstream>
using namespace std;

#include <ossimQtQuadProjectionController.h>
...
bool ossimQtQuadProjectionController::updateDialogFromImageHandler() const
{
   ...
   if (lookup)
   {
      ...
      if (type != "ossimBilinearProjection")
      {
         return false;
      }
   }
   else // no type keyword...
   {
      return false;
   }
   
   const ossim_uint32 SIZE = kwl.numberOf("gpt");
   if ( (SIZE == 0) || (SIZE != kwl.numberOf("dpt")) )
   {
      return false;
   }

   if (theDialog->theLineSampleGeographicTable->numRows() !=
       static_cast<int>(SIZE))
   {
      int rows = static_cast<int>(SIZE);
      theDialog->theLineSampleGeographicTable->setNumRows(rows);
   }

   for (ossim_uint32 i = 0; i < SIZE; ++i)
   {
      const char* gpt_lookup;
      const char* dpt_lookup;
      ossimString index_string = ossimString::toString(i);

      // Lookup the geographic point.
      ossimString kw = "gpt";
      kw += index_string;
      gpt_lookup = kwl.find(kw);

      // Lookup the line sample point.
      kw = "dpt";
      kw += index_string;
      dpt_lookup = kwl.find(kw);

      if ( gpt_lookup && dpt_lookup )
      {
         ossimGpt gp;
         istringstream gis(gpt_lookup);
         gis >> gp;
         ossimDpt dp;
         istringstream dis(dpt_lookup);
         dis >> dp;
         setTableRow(static_cast<int>(i), gp, dp);

         // Set the datum to the points datum.
         if (i ==0)
         {
            setDatumMenu(gp.datum()->code());
         }
      }
   }
   return true;
