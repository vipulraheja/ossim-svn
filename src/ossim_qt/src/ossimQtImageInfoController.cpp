...
//
//*************************************************************************
// $Id: ossimQtImageInfoController.cpp 17821 2010-08-03 13:46:38Z dburken $
...
#include <ossimQtImageInfoController.h>
#include <ossimQtImageInfoDialog.h>
#include <ossimQtQuadProjectionDialog.h>
...
{
   cout << "ossimQtImageInfoController::setupStatePlaneMenus name: "
        << name
        << std::endl;
...
void ossimQtImageInfoController::updateProjectionPanel(const ossimKeywordlist& kwl)
{
   ...
   ossimRefPtr<ossimMapProjection> proj = getNewMapProjection(kwl);
   if (proj.valid())
   {
      // Set the units menu.
      setUnitsMenu(proj.get());

      // Set the zone menu.
      setZoneMenu(proj.get());

      // Set the hemisphere.
      setHemisphereMenu(proj.get());

      // Set the origin latitude.
      setOriginLatitude(proj.get());
      
      // Set the origin latitude.
      setCentralMeridian(proj.get());

      // Set the false easting.
      setFalseEasting(proj.get());

      // Set the false easting.
      setFalseNorthing(proj.get());

      // Set the X tie point.
      setTiePointX(proj.get());

      // Set the Y tie point.
      setTiePointY(proj.get());
      ...
      setParallelOne(proj.get());

      // Set the second standard parallel.
      setParallelTwo(proj.get());
...
void ossimQtImageInfoController::buildProjectionMenu() const
{
   ...
   theDialog->theProjectionComboBox->insertStrList(projList);
}
...
ossimMapProjection* ossimQtImageInfoController::getNewMapProjection(
   const ossimKeywordlist& kwl) const
{
   ossimString projection_name = getProjectionString();

   ossimRefPtr<ossimMapProjection> map_proj =
      getNewMapProjection(projection_name);
   ...
   //---
   // Release pointer so when "map_proj" goes out of scope it doesn't self
   // destruct.
   //---
   return map_proj.release();
}

ossimMapProjection* ossimQtImageInfoController::getNewMapProjection(const ossimString& name) const
{
   ...
   if (!proj)
   {
      // Test for projection in database.
   ...
   if (proj.valid())
   {
      ...
      {
         //---
         // Note:  Since proj will go out of scope along with "map_proj"
         // ossimRefPtr::release must be called so the pointer does not self
         // destruct.
         //---
         proj.release();
      }
   ...
   return map_proj.release();  // Caller owns it...
}
...
{
   const char* lookup = kwl.find(ossimKeywordNames::ORIGIN_LATITUDE_KW);
   if (lookup)
   {
      ossimString os = lookup;
...
{
   const char* lookup = kwl.find(ossimKeywordNames::CENTRAL_MERIDIAN_KW);
   if (lookup)
   {
      ossimString os = lookup;
...
{
   const char* lookup = kwl.find(ossimKeywordNames::TIE_POINT_EASTING_KW);
   if(lookup)
   {
   ...
   {
      lookup = kwl.find(ossimKeywordNames::TIE_POINT_LON_KW);
      if (lookup)
      {
         ossimString os = lookup;
...
ossimProjection* ossimQtImageInfoController::getImageProjection() const
{
   ...
   if (trans)
   {
      ...
      if(ivpt)
      {
         ossimRefPtr<ossimImageGeometry> imageGeometry = ivpt->getImageGeometry();
         if(imageGeometry.valid())
         {
            proj = imageGeometry->getProjection();
         }
      }
   }
...
void ossimQtImageInfoController::saveProjection() const
{
   ...
   if (answer == QMessageBox::Yes)
   {
      ...
      if (ih && ic)
      {
         ...
         QFileDialog* fd = new QFileDialog( theDialog );
