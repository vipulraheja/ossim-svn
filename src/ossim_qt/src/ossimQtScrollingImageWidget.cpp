...
{
   ossimReferenced::ref(); // don't allow deletion from disconnections
   //     theBackingStoreRect.makeNan();
   theRgbChain              = new ossimImageChain;
   theEightBitRemapper      = new ossimScalarRemapper;
   theRawPixelCache         = new ossimCacheTileSource;
   
   theRgbChain->addChild(theRawPixelCache);
   theRgbChain->addChild(theEightBitRemapper);
...
void ossimQtScrollingImageWidget::drawCursor(QPainter* painter)
{
      ...
      (!isProcessingTiles()))
   {
         ...
         (theCursorLocation.y <= lr.y))
      {
         // draw horizontal
         //
         painter->drawLine(left-shiftX, theCursorLocation.y-shiftY,
                           right-shiftX, theCursorLocation.y-shiftY);
      }
      
      if((theCursorLocation.x >= ul.x)&&
         (theCursorLocation.x <= lr.x))
      {
         painter->drawLine(theCursorLocation.x-shiftX, top-shiftY,
                           theCursorLocation.x-shiftX, bottom-shiftY);
      }
