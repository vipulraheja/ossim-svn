...
void ossimQtCombinerEditorDialogController::blendSliderReleased()
{
  ...
  if(count > 1)
    {
       QListBoxItem* currentSelectedListBoxItem = theDialog->theInputWeightListBox->selectedItem();
...
void ossimQtCombinerEditorDialogController::applyChanges(bool fullRefreshFlag)
{
  ...
  {
     QListBoxItem* item = theDialog->theLayerEditor->theCurrentInputLayersListBox->item(idx);
     if(item)
     {
...
void ossimQtCombinerEditorDialogController::removingSelectedLayersFromInput(bool&)
{
   ...
   QListBox* listBox = theDialog->theLayerEditor->theCurrentInputLayersListBox;
   if(blend&&(listBox->count() > 0))
   {
      QListBoxItem* item = listBox->item((int)(listBox->count()-1));
...
void ossimQtCombinerEditorDialogController::addingSelectedLayersToInput(bool& /*canAddFlag*/)
{
   ...
   QListBox* listBox = theDialog->theLayerEditor->theAvailableLayersListBox;
   if(blend&&(listBox->count() > 0))
   {
...
void ossimQtCombinerEditorDialogController::applyBlendChanges(bool repaint)
{
  ...
  if(blend)
    {
      int currentIdx = 0;
      QListBoxItem* current = theDialog->theInputWeightListBox->firstItem();
      while(current)
	{
